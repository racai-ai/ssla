...
import ro.racai.tts.WS.NLP.utils.UnicodeBOMInputStream.BOM;
...
public class IO {
    ...
    public static BufferedWriter openFileForWriting(String filePath) throws IOException {
        return new BufferedWriter(new OutputStreamWriter(new FileOutputStream(filePath), "UTF-8"));
    }
    
    public static BufferedWriter openFileForDebugWriting(String filePath) throws IOException {
        return new BufferedWriter(new OutputStreamWriter(new FileOutputStream(filePath, true), "UTF-8"));
    }

    

    synchronized public static void outln(String out) {
        if (!verbose) {
            return;
        }
    ...
    synchronized public static void out(String out) {
        if (!verbose) {
            return;
        }
    ...
    synchronized public static void outln() {
        if (!verbose) {
            return;
        }
    ...
    synchronized public static void outln(double val) {
        if (!verbose) {
            return;
        }
        System.out.println(val); System.out.flush();
    }

    synchronized public static void errln(String err) {
        if (!verbose) {
            return;
        }
    ...
    synchronized public static void err(String err) {
        if (!verbose) {
            return;
        }
        System.err.print(err); System.err.flush();
    }

    synchronized public static void errln() {
        if (!verbose) {
            return;
        }
    ...
    synchronized public static void printHashMap (String message, HashMap<String,String> obj) {
    	IO.out(message);
    	for(String key:obj.keySet()) {
    		IO.out(" "+key+":["+obj.get(key)+"]");
    	}
    	IO.out("\n");
